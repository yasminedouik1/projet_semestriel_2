...
def prepare_input_classification(user_input):
    """
    Version intelligente : récupère automatiquement les noms de colonnes du modèle
    """
    try:
        model = joblib.load("models/classification_model.joblib")
        
        # Récupérer les noms de features attendus par le modèle
        if hasattr(model, 'feature_names_in_'):
            ...
            # Fallback : liste manuelle basée sur tes notebooks
            feature_order = [
        ...
        # Si le chargement échoue, on utilise la liste manuelle
    ...
    # Réordonner EXACTEMENT comme le modèle attend
