...
if predict_btn:
    user_input = {
        ...
        'focus_score': 50,
        'digital_dependence_score': 35.0,
        'productivity_score': 65.0,
        'physical_activity_days': 3,
        'study_mins': 100,
        # Valeurs par défaut pour les colonnes encodées / one-hot
        'gender_encoded': 0,
        'region_encoded': 0,
    ...
    # Affichage des résultats en grand
    st.success("### Résultat de l'analyse")

    col1, col2 = st.columns(2)
    with col1:
        st.metric("Probabilité de Haut Risque", f"{proba:.1%}", delta=None)
    with col2:
        if prediction == 1:
    ...
    st.info("👈 Veuillez remplir le formulaire dans la barre latérale et cliquer sur **Prédire le Risque**")