...
def load_model():
    try:
        ...
        st.error(f"Erreur de chargement du modèle classification : {e}")
        st.stop()
    ...
    if predict_btn:
        with st.container():
            ...
            user_input = {
                ...
                'anxiety_score': anxiety_score, 'stress_level': stress_level, 'sleep_hours': sleep_hours,
                'happiness_score': happiness_score, 'depression_score': depression_score,
                'focus_score': 50, 'digital_dependence_score': 35.0, 'productivity_score': 65.0,
                'physical_activity_days': 3, 'study_mins': 100,
                'gender_encoded': 0, 'region_encoded': 0, 'income_level_encoded': 2,
                'education_level_encoded': 0, 'daily_role_encoded': 1, 'device_type_encoded': 0,
                'gender_Male': 0, 'daily_role_Full-time Employee': 1,
            }
